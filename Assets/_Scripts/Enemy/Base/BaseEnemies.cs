...
public class BaseEnemies : MonoBehaviour, IDamageable
{
    ...
    void DetectPlayer()
    {
        ...
        if (CurrentState != State.Chasing)
        {
            ...
            {
                chaseDirection = -1;
                if (faceDirection != -1) Flip();
                CurrentState = State.Chasing;
            }
            else if (raycast.collider != null && raycast.collider.CompareTag("Player"))
            {
                chaseDirection = 1;
                if (faceDirection != 1) Flip();
                CurrentState = State.Chasing;
        ...
        if (CurrentState == State.Chasing && raycast.collider == null && raycast.collider == null && _loseTargetCoroutine == null)
        {
